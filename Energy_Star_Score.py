# -*- coding: utf-8 -*-
"""capstone_final_code.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1AGaOxZaMRlHc3SKjxyYpno_NnX1ZawU0
...
In this project we are gonna predict the Energy start score of a building unsing Machine learning techniques.
 ...
 we are working on a supervised, regression machine learning problem. Using real-world New York City building energy data, we want to predict the Energy Star Score of a building and determine the factors that influence the score.
...
data = pd.read_csv('/content/Energy_and_Water_Data_Disclosure_for_Local_Law_84_2017__Data_for_Calendar_Year_2016_.csv')
data.head()
...
"""replacing the missing values with NA and converting data to correct types"""
...
If we go back to the definition of the score, we see it is based on self-reported energy usage. This poses a problem, because a building owner might be tempted to report lower electricity usage to artificially boost the score of their building. 
...
In order to look at the effect of categorical variables on the score, we can make a density plot colored by the value of the categorical variable. Density plots also show the distribution of a single variable and can be thought of as a smoothed histogram. 
...
In part two, we will focus on implementing several machine learning methods, selecting the best model, and optimizing it for our problem using hyperparameter tuning with cross validation. As a final step here, we will save the datasets we developed to use again in the part 2.
...
2. Perform hyperparameter tuning on the best model to optimize it for the problem
...
Again, here I'm focusing on implementation rather than explaining how these work.
...
The Randomized Search Object is trained the same way as any other scikit-learn model. After training, we can compare all the different hyperparameter combinations and find the best
"""
...
In the final part, we will try to answer these questions and draw final conclusions from the project!
